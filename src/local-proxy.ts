...
import { createWalletClient, createPublicClient, http as viemHttp, parseUnits, formatUnits, encodeFunctionData } from 'viem';
...
let defaultSeller: string | null = '0x3137cE5612af147f1BA17eBba7e8B46594ed3e26'; // 默认卖家
...
// ============ Gateway 交互 ============
async function fetchSellerAddress(): Promise<string> {
...
async function checkSellerActive(sellerAddress: `0x${string}`): Promise<boolean> {
  try {
...
async function ensureApproval(): Promise<void> {
  const allowance = await publicClient.readContract({
    ...
    args: [wallet!.address as `0x${string}`, CHANNEL_CONTRACT]
  });
  ...
  try {
    ...
    for (let i = logs.length - 1; i >= 0; i--) {
      ...
      if (logBuyer.toLowerCase() === buyerAddress && logSeller.toLowerCase() === sellerAddress.toLowerCase()) {
        ...
        const deposit = channelData[2] as bigint;
        const settledAmount = channelData[3] as bigint;
        const expiresAt = Number(channelData[4] as bigint);

        if (expiresAt < Math.floor(Date.now() / 1000)) continue; // 已过期
...
// ============ Ticket 签名 ============
async function signTicket(channel: ChannelInfo, amount: bigint): Promise<string> {
  channel.nonce += 1n;
...
function proxyRequest(
  ...
  body: string, 
  ticket?: { channelId: string; amount: string; nonce: string; signature: string }
) {
  const url = new URL(GATEWAY_URL);
  ...
  if (req.method === 'POST' && (url === '/v1/messages' || url === '/messages')) {
    ...
    req.on('end', async () => {
      try {
        ...
        if (!channel) {
          try {
            channel = await openChannel(sellerAddress as `0x${string}`);
          } catch (e: any) {
