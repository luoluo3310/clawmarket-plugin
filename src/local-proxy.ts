...
const USDC_CONTRACT = '0xcF0819eb156D6c6c1c5d9A515E351D2D1aefff7D' as `0x${string}`;
const CHANNEL_CONTRACT = '0x1577e78D8a446edF10244A80bEf990751e80E495' as `0x${string}`;
const MIN_BALANCE = 1_000_000n;
const CHANNEL_DEPOSIT = 10_000_000n;
const CHANNEL_DURATION = BigInt(7 * 24 * 60 * 60);
const COST_PER_REQUEST = 100_000n; // 初始 ticket 0.1 USDC（预授权）
...
// ============ 类型 ============
interface WalletData { address: string; privateKey: string; createdAt: string; }
interface ChannelInfo {
  channelId: `0x${string}`; seller: string; deposit: bigint;
  spent: bigint; nonce: bigint; expiresAt: number;
}
...
let channels: Map<string, ChannelInfo> = new Map();
let defaultSeller: string = '0x3137cE5612af147f1BA17eBba7e8B46594ed3e26';
// 默认卖家在 Registry 的 ID（启动时获取）
let defaultSellerRegistryId: string | null = null;

// ============ 钱包管理 ============
function loadOrCreateWallet(): WalletData {
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
  if (fs.existsSync(WALLET_PATH)) return JSON.parse(fs.readFileSync(WALLET_PATH, 'utf-8'));
  const privateKey = generatePrivateKey();
  const account = privateKeyToAccount(privateKey);
  const w: WalletData = { address: account.address, privateKey, createdAt: new Date().toISOString() };
...
async function getBalance(): Promise<bigint> {
  return publicClient.readContract({ address: USDC_CONTRACT, abi: USDC_ABI, functionName: 'balanceOf', args: [wallet!.address as `0x${string}`] });
}
async function getETHBalance(): Promise<bigint> {
  return publicClient.getBalance({ address: wallet!.address as `0x${string}` });
}

async function ensureApproval(): Promise<void> {
  const allowance = await publicClient.readContract({ address: USDC_CONTRACT, abi: USDC_ABI, functionName: 'allowance', args: [wallet!.address as `0x${string}`, CHANNEL_CONTRACT] });
...
async function recoverChannelFromChain(sellerAddress: `0x${string}`): Promise<ChannelInfo | null> {
  try {
    ...
    for (let i = logs.length - 1; i >= 0; i--) {
      ...
      if (logBuyer.toLowerCase() === buyerAddress && logSeller.toLowerCase() === sellerAddress.toLowerCase()) {
        const channelId = log.topics[1] as `0x${string}`;
        ...
        if (!channelData[6]) continue;
        const expiresAt = Number(channelData[4] as bigint);
        if (expiresAt < Math.floor(Date.now() / 1000)) continue;
        const channel: ChannelInfo = { channelId, seller: sellerAddress, deposit: channelData[2] as bigint, spent: channelData[3] as bigint, nonce: 0n, expiresAt };
  ...
  } catch (e: any) { console.log('[恢复] 失败:', e.message); }
  return null;
}

async function openChannel(sellerAddress: `0x${string}`): Promise<ChannelInfo> {
  console.log('[链上] 开通道到:', sellerAddress);
  const recovered = await recoverChannelFromChain(sellerAddress);
  if (recovered) return recovered;
  const ethBalance = await getETHBalance();
  if (ethBalance < parseUnits('0.001', 18)) throw new Error('ETH 不足');
  const usdcBalance = await getBalance();
  if (usdcBalance < CHANNEL_DEPOSIT) throw new Error(`USDC 不足: ${formatUnits(usdcBalance, 6)}`);
  if (SKIP_SELLER_STAKE_CHECK) console.log('[测试] 跳过 stake 检查');
  ...
  let channelId: `0x${string}` = `0x${'0'.repeat(64)}` as `0x${string}`;
  for (const log of receipt.logs) {
    if (log.address.toLowerCase() === CHANNEL_CONTRACT.toLowerCase() && log.topics.length > 1) {
      channelId = log.topics[1] as `0x${string}`;
      break;
    }
  }
  const expiresAt = Math.floor(Date.now() / 1000) + Number(CHANNEL_DURATION);
  const channel: ChannelInfo = { channelId, seller: sellerAddress, deposit: CHANNEL_DEPOSIT, spent: 0n, nonce: 0n, expiresAt };
  channels.set(sellerAddress.toLowerCase(), channel);
  saveChannels();
...
// ============ Ticket 签名 ============
async function signTicket(channel: ChannelInfo, amount: bigint): Promise<{ channelId: string; amount: string; nonce: string; signature: string; buyer: string }> {
  channel.nonce += 1n;
...
// ============ 获取卖家 Registry ID ============
async function fetchSellerRegistryId(): Promise<string | null> {
    ...
    http.get(`${GATEWAY_URL}/api/market`, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (result.sellers?.length > 0) resolve(result.sellers[0].id);
          else resolve(null);
        } catch { resolve(null); }
      });
 ...
 * 返回 SSE 给 OpenClaw
 */
async function handleStreamRequest(req: http.IncomingMessage, res: http.ServerResponse, body: string) {
  const request = JSON.parse(body);
  console.log(`[请求] Model: ${request.model}, Stream: true`);

  // 确保有通道和 ticket
  const sellerAddress = defaultSeller.toLowerCase() as `0x${string}`;
  ...
  if (!channel) {
    try { channel = await openChannel(sellerAddress as `0x${string}`); }
    catch (e: any) {
...
function proxyRequestHTTP(req: http.IncomingMessage, res: http.ServerResponse, body: string, ticket?: any) {
  const url = new URL(GATEWAY_URL);
  const mod = url.protocol === 'https:' ? https : http;
  const headers: Record<string, string> = { 'Content-Type': 'application/json', 'x-api-key': wallet!.address, 'anthropic-version': '2023-06-01' };
  if (ticket) {
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  if (req.method === 'POST' && (url === '/v1/messages' || url === '/messages')) {
    ...
    req.on('end', async () => {
      try {
        ...
        let channel = channels.get(sellerAddress);
        if (channel && (channel.expiresAt < Date.now() / 1000 || channel.spent + COST_PER_REQUEST > channel.deposit)) {
          channels.delete(sellerAddress); channel = undefined;
        }
        if (!channel) {
          try { channel = await openChannel(sellerAddress as `0x${string}`); } catch {}
        }
  ...
  if (url === '/open-channel') {
    try {
      const channel = await openChannel(defaultSeller as `0x${string}`);
      res.writeHead(200, { 'Content-Type': 'application/json' });
...
async function main() {
  ...
  try {
    ...
    if (balance < MIN_BALANCE) console.log('⚠️  USDC 不足，请充值到:', wallet.address);
  } catch (e: any) { console.error('[余额] 查询失败:', e.message); }

  // 预获取卖家 Registry ID
  defaultSellerRegistryId = await fetchSellerRegistryId();
  if (defaultSellerRegistryId) console.log('[卖家] Registry ID:', defaultSellerRegistryId);
  else console.log('[卖家] ⚠️ 未找到在线卖家');
