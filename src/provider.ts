...
const DEFAULT_CONFIG = {
  ...
  chainId: 84532,
  minBalance: 1_000_000n, // 1 USDC minimum to start
...
export class ClawMarketProvider {
   ...
   * 初始化 Provider
   */
  async initialize(): Promise<{ address: string; balance: string; ready: boolean }> {
    // 1. 加载或创建钱包
  ...
  async refreshSellers(): Promise<void> {
    try {
      ...
      const data = await res.json() as { sellers: SellerInfo[] };
  ...
  selectSeller(model: string): SellerInfo | null {
    const candidates = this.sellers.filter(s => 
   ...
   * 确保与卖家有通道
   */
  async ensureChannel(seller: SellerInfo): Promise<Channel> {
    ...
    const sellerAddress = ('0x' + seller.public_key.slice(2, 42)) as `0x${string}`;
    ...
    const channelId = receipt.logs[1]?.topics[1] as `0x${string}`;
   ...
   * 签名票据
   */
  async signTicket(channel: Channel, amount: bigint): Promise<string> {
    channel.nonce += 1n;
  ...
  async chat(params: {
    ...
    const result = await res.json() as { success: boolean; payload?: string; error?: string };
   ...
   * 获取状态
   */
  getStatus(): { address: string; ready: boolean; sellers: number; channels: number } {
    return {
      address: this.wallet?.address || '',
      ready: this.isReady,
   ...
   * 等待余额充足
   */
  async waitForBalance(intervalMs = 10000): Promise<void> {
...
// 导出便捷函数
export async function initClawMarket(config?: Partial<typeof DEFAULT_CONFIG>) {
