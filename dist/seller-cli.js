...
function saveSellerConfig(config) {
...
function isSellerRunning() {
    if (!fs.existsSync(SELLER_PID_PATH))
        return false;
 ...
 * 注册到 Registry
 */
async function registerSeller(config) {
...
async function updatePrice() {
    ...
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--input' && args[i + 1])
            inputPrice = parseFloat(args[i + 1]);
        if (args[i] === '--output' && args[i + 1])
            outputPrice = parseFloat(args[i + 1]);
    }
 ...
 * 卖家 CLI 主入口
 */
export async function sellerMain(subcommand) {
    console.log('');
    console.log('╔═══════════════════════════════════════════════════════════╗');
...
//# sourceMappingURL=seller-cli.js.map